...
extern struct timespec task_time_start_1, task_time_end_1,task_time_start_2[FaRM_buffer_NUM+4];
extern unsigned long task_time_diff_ns_1;
...
#ifdef RFP
	if(server!= NULL)
		server->L5_message[rdma_trans->node_id].addr = NULL;
#endif
#ifdef FaRM
...
task_time_diff_ns_1 = task_time_diff_ns_1 + ((task_time_end_1.tv_sec * 1000000000) + task_time_end_1.tv_nsec) -
		...
		dhmp_post_send(rdma_trans, &res_msg);
	INFO_LOG("reply size = %d",reply_size); 
#ifdef SFLUSH
	context_ptr = msg->data + sizeof(struct dhmp_DaRPC_request) + 2;
	for(i = 0;i < batch;i++)
	{
		server_addr= (void *)*(uintptr_t*)context_ptr;
		size = *(size_t*)(context_ptr + sizeof(uintptr_t));
...
void amper_DaRPC_response_handler(struct dhmp_transport* rdma_trans, struct dhmp_msg* msg) // read
{
	...
	if(response_msg->write_flag == 0)
	{
		void* temp;
		int i;
		temp = response_msg + sizeof(struct dhmp_DaRPC_response);
...
void dhmp_octo_request_handler(struct dhmp_transport* rdma_trans)
{
	...
	memcpy (response , server->octo[rdma_trans->node_id].S_mr->addr , sizeof(struct dhmp_octo_request));
	if(write_flag == 1)
	{
		memcpy(server_addr, context_ptr, size);
		_mm_clflush(server_addr);
	...
	INFO_LOG("remotemr %p localmr %p lentgh=%d", server->octo[rdma_trans->node_id].C_mr.addr,server->octo[rdma_trans->node_id].local_mr->addr,reply_size);
