...
if(flag_write > 10)
	...
	{
		pthread_mutex_lock(&client->mutex_request_num);
		if(client->FaRM.is_available[client->FaRM.Scur] != 1)
                {
                        pthread_mutex_unlock(&client->mutex_request_num);
                        continue;
                }
                if(client->FaRM.is_available[(client->FaRM.Scur+FaRM_buffer_NUM-1)%FaRM_buffer_NUM] == 2)
                {
			pthread_mutex_unlock(&client->mutex_request_num);
                        continue;
                }	
		...
		wwork.cur = client->FaRM.Scur;
#ifdef WFLUSH
		 if(flag_write == 0) client->FaRM.is_available[client->FaRM.Scur] = 2;
...
void dhmp_client_init(size_t size,int obj_num,int writepart)
{
	...
	char * realname = malloc(120);
	sprintf(realname,"result_%d_%d.txt",size,writepart);
#ifdef taillantency
	fp = fopen(realname,"w+");
#endif
...
if(flag_write > 10)
        flag_write = flag_write - 10;
        sleep(0);
	amper_sendRPC_DaRPC(globle_addr, length, local_addr,3 , batch);
#endif
...
void model_7_scalable( uintptr_t * globle_addr , size_t length, uintptr_t * local_addr, char flag_write, char batch)
{
	...
	if(client->scaleRPC.context_swith == Sca1e_Swith_Time)
	{
		...
		for(i = 0; i < batch ;i++)
		{
			...
			memcpy(local_data + sizeof(uintptr_t) , &(local_addr[0]) , sizeof(uintptr_t));
	...
	else
	{
		...
		for(i = 0; i < batch ;i++)
		{
			...
			memcpy(scalable_write + sizeof(uintptr_t) , &(local_addr[0]) , sizeof(uintptr_t));
